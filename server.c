...
#endif
	return apolloParsePacket(bev, (client_t *)arg);
}
...
void on_accept(int fd, short ev, void *arg) {
	...
	if ((client->pBlock = malloc(sizeof(BlockBuffer))) == NULL) {
		warn("client event_base creation failed");
		closeAndFreeClient(client);
		return;
	}
	memset(client->pBlock, 0, sizeof(BlockBuffer));
